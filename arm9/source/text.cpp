    ...
    switch (textVars.curMode) {
        ...
        case TEXTMODE_WINDOW_DIRDOWN:
            ...
            if (textVars.scroolY > 52) {
                ...
                for (list<pFigure>::iterator i = activeFigures.begin(); i != activeFigures.end();
                     ++i) {
                    pFigure curFigure = *i;
...
{
    textVars.textMap[posX + posY * 32] =
        (textVars.txtBuffer[bufferPos] - 32) | (textVars.curColor << 12);
}
...
{
    const char *attrs[] = { "STR", "DEF", "AGI", "VIT", "INT", "DEX" };
    ...
    for (int i = STR; i < DEX + 1; ++i) {
        ...
        if (deltaAttr > item->itemChanges[i])
            putText(22, 1 + (i - STR), "%s:\2%2d\2 ", attrs[i - STR],
                    tempFigure.Attr[i] + item->itemChanges[i]);
        else if (deltaAttr < item->itemChanges[i])
            putText(22, 1 + (i - STR), "%s:\3%2d\3 ", attrs[i - STR],
                    tempFigure.Attr[i] + item->itemChanges[i]);
        else
            putText(22, 1 + (i - STR), "%s:%2d ", attrs[i - STR],
                    tempFigure.Attr[i] + item->itemChanges[i]);
    }
}
