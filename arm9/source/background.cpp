...
Map levelMaps[LEVEL_SIZE*3] = {
	...
	levelData(4), levelData(5), levelData(6),
	levelData(7), levelData(8), levelData(9),
	levelData(10), levelData(11), levelData(12),
	levelData(13), levelData(14), levelData(15),
	levelData(16), levelData(17), levelData(18),
};

u16* levelPalettes[LEVEL_SIZE] = {
	levelPal(1), levelPal(2), levelPal(3),
	levelPal(4), levelPal(5), levelPal(6),
	levelPal(7), levelPal(8), levelPal(9),
	levelPal(10), levelPal(11), levelPal(12),
	levelPal(13), levelPal(14), levelPal(15),
	levelPal(16), levelPal(17), levelPal(18),
};

u8* levelCollisions[LEVEL_SIZE] = {
	levelCollision(1), levelCollision(2), levelCollision(3),
	levelCollision(4), levelCollision(5), levelCollision(6),
	levelCollision(7), levelCollision(8), levelCollision(9), 
	levelCollision(10), levelCollision(11), levelCollision(12),
	levelCollision(13), levelCollision(14), levelCollision(15), 
	levelCollision(16), levelCollision(17), levelCollision(18),
};

const u32 levelSizes[LEVEL_SIZE*2] = {
	480, 480, 480, 320, 256, 192, 256, 192, 320, 320,
	1280, 1280, 1280, 1280, 1280, 1280, 256, 192, 256, 192,
	1280, 1280, 1280, 1280,  320, 256, 320, 320,  320, 320,
	1280, 1280,  320, 320,  416, 416, 
};
/*
...
const u8* battleMapsPals[LEVEL_SIZE*2] = {
	NULL, NULL,
	NULL, NULL,
	battleCastle_bin, battleCastlePal_bin,
	NULL, NULL,
	NULL, NULL,
	battleBeach_bin, battleBeachPal_bin,
	battleTown_bin, battleTownPal_bin,
	battleWoods_bin, battleWoodsPal_bin,
	NULL, NULL,
	NULL, NULL,
	battleWoods_bin, battleWoodsPal_bin,
	battleBeach_bin, battleBeachPal_bin,
	NULL, NULL,
	battleICave_bin, battleICavePal_bin,
	battleFCave_bin, battleFCavePal_bin,
	battleWoods_bin, battleWoodsPal_bin, 
	battleECave_bin, battleECavePal_bin,
	battleECastle_bin, battleECastlePal_bin,
};
...
/*	if(curLevel->isAnimated) {
		if(frameCounter%12 == 0)
			++curLevel->animIndex;
		dmaCopy(curLevel->animData[curLevel->animIndex%4]->tileData, (void*)BG_TILE_RAM(0), curLevel->animData[curLevel->animIndex%4]->tileSize);
		backgroundUpdateView((u16*)BG_MAP_RAM(31), (u16*)curLevel->animData[curLevel->animIndex%4]->mapData);
	} else */
...
void backgroundUpdateView(u16* workMap, const u16* targetMap)
{
	...
	for(u32 x = 0; x < 32; x++) {
		for(u32 y = 0; y < 32; y++) {
			workMap[x + y*32] = targetMap[(x+curLevel->viewX/8) + (y+curLevel->viewY/8)*(curLevel->dimX/8)];
		}
	}
		// Second chunk map
	workMap += 32*31;
...
{
		//Set to MODE0 and Sprites
	videoSetMode(MODE_0_2D | DISPLAY_BG0_ACTIVE | DISPLAY_BG2_ACTIVE | DISPLAY_BG3_ACTIVE | DISPLAY_SPR_1D | DISPLAY_SPR_ACTIVE);
...
void updateGauge(pGauge workGauge)
{
	...
	dmaCopy(battleGauge_bin, SPRITE_GFX_SUB + workGauge->dataOffset*16, 32);
	dmaCopy(battleGauge_bin + (modulo+1)*32, SPRITE_GFX_SUB + ((workGauge->dataOffset+1) + devisor)*16, 32);
	if(devisor)
		dmaCopy(battleGauge_bin + 9*32, SPRITE_GFX_SUB + ((workGauge->dataOffset+1) + devisor-1)*16, 32);
	dmaCopy(battleGauge_bin + 32*10, SPRITE_GFX_SUB + (workGauge->dataOffset+7)*16, 32);
}
...
void initMode(gameMode selMode)
{
	...
	switch(selMode) {
		...
		case LEVELMODE_MENU_LOAD:
				...
				fillSaves();
				for(int i=0; i<3; ++i)
					textPutOn(true, (u16*)BG_MAP_RAM(31), 6, 6 + i*6, gameValues.saveSlots[i].saveName);
			break;
		...
		case LEVELMODE_BATTLE_FADEPAUSE:
				...
				for(int i=0; i<32*32; ++i) ((u16*)BG_MAP_RAM(31))[i] = i;
				curLevel = &battleLevel; // NOT loadLevel!
				initGauge(&battleValues.playerG, 188,  148 + 8, curPlayer->Attr[AGI]);
				initGauge(&battleValues.mnsGuage, 8*8 - 4,  148 + 8, battleValues.curMonster.Attr[AGI]);
				hideSprite(battleValues.playerG.indSpr, true);
...
bool saveGame(int saveSlot)
{
	...
	for(int i=0; i< (int)curPlayer->figureItemList.size(); ++i) 
		gameData.playerItems[curPlayer->figureItemList[i].itemIdx] = curPlayer->figureItemList[i].quantity;
