...
void updateSpriteData(pFigure theFigure)
{
    ...
    const u8 *sourceImage = theFigure->allData
                            + ((32 * 48 / 2) * theFigure->animSize * theFigure->spriteDirIndex)
                            + (32 * 48 * (theFigure->spriteIndex % theFigure->animSize)) / 2;
...
void loadNPCs()
{
    ...
    while (gameNPCs[npcIndex].Figure != NULL) {
        initFigure(gameNPCs[npcIndex].npcName, gameNPCs[npcIndex].Figure,
                   gameNPCs[npcIndex].npcData, gameNPCs[npcIndex].npcPalette);
        ...
        if (gameNPCs[npcIndex].moveX | gameNPCs[npcIndex].moveY)
            moveFigure(gameNPCs[npcIndex].Figure,
                       gameNPCs[npcIndex].posX + gameNPCs[npcIndex].moveX,
                       gameNPCs[npcIndex].posY + gameNPCs[npcIndex].moveY, true);
...
void loadChests()
{
    ...
    while (loadedChests[curChest].chestDir != DIR_NONE) {
        initChest(&chestFigure[curChest], loadedChests[curChest].gfxId,
                  loadedChests[curChest].itemIdx, loadedChests[curChest].quantity);
        setPosFigure(&chestFigure[curChest].cFigure, loadedChests[curChest].posX,
                     loadedChests[curChest].posY);
...
void upadteWeapon(pWeapon cWeapon)
{
    ...
    if (frameCounter % 8 == 0) {
        ...
        dmaCopy(cWeapon->dataPtr + (cWeapon->animIdx % 3) * 512, SPRITE_GFX + dataOffset * 16,
                32 * 32 / 2);
