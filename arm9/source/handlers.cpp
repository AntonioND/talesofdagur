	...
	switch(gameValues.curMode) {
		...
		case LEVELMODE_PLAY:
						...
						if(curPlayer->posX/8 >= curLevel->linkedLevels[curLinkedLevel].startTile) {
							if(curPlayer->posX/8 <= curLevel->linkedLevels[curLinkedLevel].endTile) {
								if(curLevel->linkedLevels[curLinkedLevel].wallDir == DIR_DOWN) {
									...
									gameValues.newPosX = curLevel->linkedLevels[curLinkedLevel].posFix * 8;
						...
						if(curPlayer->posX/8 >= curLevel->linkedLevels[curLinkedLevel].startTile) {
							if(curPlayer->posX/8 <= curLevel->linkedLevels[curLinkedLevel].endTile) {
								if(curLevel->linkedLevels[curLinkedLevel].wallDir == DIR_UP) {
									levelLoad  = curLevel->linkedLevels[curLinkedLevel].loadedLevel;
									gameValues.newPosY = Levels[curLevel->linkedLevels[curLinkedLevel].loadedLevel].dimY - 32; 
									gameValues.newPosX = curLevel->linkedLevels[curLinkedLevel].posFix * 8;
									gameValues.newDir = curLevel->linkedLevels[curLinkedLevel].wallDir;
									break;
								}
				...
				if(curPlayer->posX < -8) {
					int curLinkedLevel = 0;
					while(curLevel->linkedLevels[curLinkedLevel].loadedLevel != NO_LEVEL) {
						if((curPlayer->posY+32)/8 >= curLevel->linkedLevels[curLinkedLevel].startTile) {
							if((curPlayer->posY+32)/8 <= curLevel->linkedLevels[curLinkedLevel].endTile) {
								if(curLevel->linkedLevels[curLinkedLevel].wallDir == DIR_LEFT) {
									levelLoad  = curLevel->linkedLevels[curLinkedLevel].loadedLevel;
									gameValues.newPosX = Levels[curLevel->linkedLevels[curLinkedLevel].loadedLevel].dimX - 24;
									gameValues.newPosY = curLevel->linkedLevels[curLinkedLevel].posFix * 8;
									gameValues.newDir = curLevel->linkedLevels[curLinkedLevel].wallDir;
									break;
								}
							}
		...
		case LEVELMODE_GAMEOVER_WHITEFADE:
				...
				if(gameValues.fadeVal > 15) {
					...
					REG_IME = 0; 
					SndStopMOD();
					for(int i=0; i<32*32; i++) ((u16*)BG_MAP_RAM_SUB(31))[i] = 0;
...
{
	const char* equips[6] = { "Weapon: ", "Head: ", "Armor: ",  "Shield: ", "Access.: ", "Orb: ", };
	...
	switch(textVars.curOption) {
		case EQUIP:
				for(int i=WEAPON; i<=ORB; ++i) {
					if(curPlayer->equipedItems[i] != -1)
						menuText(2, 2 + (i-WEAPON), "%s%s", equips[i-WEAPON], itemDB[curPlayer->equipedItems[i]].itemName);
					else
		...
		case ITEM:
				...
				for(int i=0; i < (int)curPlayer->figureItemList.size(); ++i)
					textVars.subMenus[textVars.curOption].push_back(curPlayer->figureItemList[i].itemData->itemName);
				...
				for(int i=textVars.virtualPosY; i < minSize; ++i) {
					if(textVars.curOption == ITEM) {
						if(curPlayer->figureItemList[i].itemData->itemType == POTION) {
							menuText(12, 11 + (i-textVars.virtualPosY), "%s", curPlayer->figureItemList[i].itemData->itemName);
							menuText(26, 11 + (i-textVars.virtualPosY), "%2d", curPlayer->figureItemList[i].quantity);
						} else {
							menuText(12, 11 + (i-textVars.virtualPosY), "\5%s\5", curPlayer->figureItemList[i].itemData->itemName);
							menuText(26, 11 + (i-textVars.virtualPosY), "\5%2d\5", curPlayer->figureItemList[i].quantity);
						}
					} else {
						if(curPlayer->figureItemList[i].itemData->itemType != POTION) {
							menuText(12, 11 + (i-textVars.virtualPosY), "%s", curPlayer->figureItemList[i].itemData->itemName);
				...
				if(!curPlayer->figureItemList[textVars.menuSel + textVars.virtualPosY].equiped) {
					...
					writeStaticsWith(curPlayer->figureItemList[textVars.menuSel + textVars.virtualPosY].itemData);
				} else
					textVars.writeStatics = true;
					
				menuText(12, 11 + textVars.menuSel, "\4%s\4", curPlayer->figureItemList[textVars.menuSel + textVars.virtualPosY].itemData->itemName);
				menuText(26, 11 + textVars.menuSel, "\4%2d\4", curPlayer->figureItemList[textVars.menuSel + textVars.virtualPosY].quantity);
		...
		case MAIN_MENU:
				...
				menuText(2, 16 + textVars.menuSel, "\4%s\4", textVars.smallMenu[textVars.menuSel].c_str());
			break;
	}
}
				...
				case ITEM:
						if(!curPlayer->figureItemList.empty()) {
							if(curPlayer->figureItemList[textVars.menuSel].itemData->itemType == POTION)
								useItem(curPlayer, curPlayer->figureItemList[textVars.menuSel + textVars.virtualPosY].itemIdx);
						}
					break;
				case MAGIC:
					break;
...
void handleShop()
{
	...
	switch(curOption) {
		...
		case OPTION_BUY:
				...
				if(KEYS & KEY_DOWN) {
					for(int i=0; i<32*6; ++i) ((u16*)BG_MAP_RAM_SUB(2))[i] = 9;
		...
		case OPTION_SELL: 
				...
				for(int i=virtualPosY; i < minSize; ++i) {
					if(i == textVars.menuSel + virtualPosY) {
						menuText(12, 12 + (i - virtualPosY), "\4%-12s\4", curPlayer->figureItemList[i].itemData->itemName);
						menuText(27, 12 + (i - virtualPosY), "\4%4d\4", curPlayer->figureItemList[i].itemData->itemPrice/2);
					} else {
						menuText(12, 12 + (i - virtualPosY), "%-12s", curPlayer->figureItemList[i].itemData->itemName);
						menuText(27, 12 + (i - virtualPosY), "%4d", curPlayer->figureItemList[i].itemData->itemPrice/2);
					}
				}
				...
				if(KEYS & KEY_A) {
					...
					if( !curPlayer->figureItemList[textVars.menuSel + virtualPosY].equiped ) {
						removeItem(curPlayer, curPlayer->figureItemList[textVars.menuSel + virtualPosY].itemIdx, 1);
						curPlayer->Attr[GOLD] += curPlayer->figureItemList[textVars.menuSel + virtualPosY].itemData->itemPrice / 2;
...
void handleBattleChoice(int battleSel)
{
	switch(battleSel) {
		...
		case 2: // Item	
				...
				for(itemList::iterator i=curPlayer->figureItemList.begin(); i!=curPlayer->figureItemList.end(); ++i) {
	...
	switch(battleValues.curMode) {
		...
		case BATTLE_PLAYERCHOICE_MONSTER:
				...
				if(KEYS & KEY_A) {
					...
					if(curPlayer->equipedItems[WEAPON] != -1) {
						initWeapon(&battleValues.activeWeapon, itemDB[curPlayer->equipedItems[WEAPON]].itemType);
		...
		case BATTLE_CHOOSEITEM:
				...
				if(KEYS & KEY_A) {
					...
					battleValues.hpDec = battleValues.battleItems[textVars.menuSel].itemData->itemChanges[HP];
...
{
	const char* battleMenu[] = { "Attack", "\5Magic\5", "Item", "Flee", };
	if(battleValues.isBoss)
	...
	switch(battleValues.curMode) {
		...
		case BATTLE_MONSTERACTION:
				if(!battleValues.isSet) {
					...
					battleValues.hpDec += rand()%(battleValues.curMonster.Attr[DEX] + battleValues.curMonster.Attr[AGI]);
			...
			break;
		case BATTLE_ELIMINATEMONSTER:
				rotScaleSprite(&spriteRotationsMain[0], gameValues.flowerAngle+=16, gameValues.textZoom+=64);
		...
		case BATTLE_VICTORY:
				if(textVars.curMode == TEXTMODE_IDLE) {
					if(!battleValues.isSet) {
						...
						menuText(5, 14, "EXP To next level : %d", curPlayer->Attr[EXP_TO_NEXT] - curPlayer->Attr[EXP]);
						if(battleValues.deltaLevel)
