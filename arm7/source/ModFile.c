...
static void LoadSamples(MODFILE_LOAD_VARS *vars)
{
    ...
    for (i = 0; i < 31; i++) {
        ...
        smp->length = (((smp->length & 0xff) << 8) | (smp->length >> 8));

        smp->loopStart = (((smp->loopStart & 0xff) << 8) | (smp->loopStart >> 8));

        smp->loopLength = (((smp->loopLength & 0xff) << 8) | (smp->loopLength >> 8));
        ...
        for (row = 0; row < 64; row++) {
            for (column = 0; column < sndMod.channelCount; column++) {
                ...
                *((u32 *)cell) = *((u32 *)vars->src);
                ...
                u32 *outCell =
                    (u32 *)&vars->dest
                        ->pattern[curPattern][row * vars->dest->channelCount * 4 + column * 4];
                *outCell = closestNote | (sample << 8) | (effect << 16) | (param << 24);
            }
        }
    }
...
static void LoadSampleDatas(MODFILE_LOAD_VARS *vars)
{
    ...
    for (curSmp = 0; curSmp < 31; curSmp++) {
        ...
        if (smp->length > 0) {
            ...
            if (smp->length & 1) {
                smp->smpData[smp->length * 2 + 1] = smp->smpData[smp->length * 2] =
