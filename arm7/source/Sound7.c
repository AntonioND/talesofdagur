...
static void SndCmdSetMemPool(SND_COMMAND *cmd)
{
    ...
    sndVars.memPoolSize = cmd->param[0] | (cmd->param[1] << 8) | (cmd->param[2] << 16);
}
...
static void MODHandleUpdateFlags(MOD_UPDATE_VARS *vars)
{
    ...
    if (vars->sndChn->data == NULL) {
        ...
        SCHANNEL_CR(vars->chnIdx) =
            SOUND_VOL(vars->sndChn->vol) | SOUND_PAN(vars->sndChn->pan)
            | (vars->sndChn->loopLength == 0 ? SOUND_ONE_SHOT : SOUND_REPEAT) | SOUND_FORMAT_8BIT
            | schnEnable;
    }
...
static void MODFXPortaDownMid(MOD_UPDATE_VARS *vars)
{
    vars->modChn->period = MODPitchSlide(vars->modChn->period, vars->modChn->portaSpeed);
